...
if __name__=="__main__":
    ...
    pickle.dump(cv,open("data/cv_model.pkl","wb"))
    pickle.dump(tfidf,open("data/tfidf_model.pkl","wb"))
    pickle.dump(wordnet,open("data/wordnet.pkl","wb"))
    pickle.dump(nb_model, open("data/nb_model.pkl", "wb"))
    print(nb_model.score(X_counts_tfidf_arr_train,y_train),\
          nb_model.score(X_counts_tfidf_arr_test,y_test))
    message = "I really hate this movie"
    df_try = pd.DataFrame([message],columns=["Review"])
    df_try_clean = cleaned_dframe(df_try.copy(),"Review")
    X_counts_try, X_counts_tfidf_arr_try = text2num(df_try_clean.copy(),\
                                                      "text",False,cv,tfidf)
    print(int(100*np.around(nb_model.predict_proba(X_counts_tfidf_arr_try)[0,1],decimals=2)))
